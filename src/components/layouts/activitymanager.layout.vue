      ...
      >
        ...
        <div class="flex items-center justify-between w-full lg:w-auto">
          ...
          <span class="font-bold text-white mx-4 hidden lg:block"
            >WFP CASE TRACKER | Activity Manager
...
onMounted(() => {
  ...
  eventBus.on("ProcessArchived", (lpId) => {
    getProcess();
    updateNotifications();
  });
});
...
const updateNotifications = () => {
  ...
  if (newEscalatedCaseCount.value > 0) {
    notifications.value.push({
      ...
      href: "/activitymanager/cases/escalated",
    });
  }

  if (newEscalatedProcessCount.value > 0) {
    notifications.value.push({
      message: `New Issues (${newEscalatedProcessCount.value})`,
      href: "/activitymanager/process",
    });
  }
};
function navigation() {
  let navList = [
    {
      name: "Home",
      href: "/activitymanager/dashboard",
      icon: HomeIcon,
      current: false,
    },
    {
      name: "CFM",
      href: "/activitymanager/cases",
    ...
    {
      name: "Resources",
      href: "/activitymanager/resources",
      icon: IdentificationIcon,
      current: false,
    },
  ];
  ...
  navList.forEach((navItem) => {
    const isMatched =
      ...
      (navItem.name === "Resources" &&
        (currentRouteBase.startsWith("/activitymanager/faqs") ||
          currentRouteBase.startsWith("/activitymanager/sops") ||
...
const onAbout = async () => {
  $router.push({ path: "/activitymanager/about-system" });
};

const newEscalatedCaseCount = ref(0);
const newEscalatedProcessCount = ref(0);
const getCases = async () => {
  try {
    ...
    cases.push(
      ...result.filter(
        (item) =>
          item.isEscalated == true && item.escalatedTo == "Activity Manager"
      )
    );
...
const getProcess = async () => {
  try {
    ...
    processR.push(
      ...result.filter((item) => item.activity == user.value.activity)
    );
    newEscalatedProcessCount.value = processR.length;
    updateNotifications();
  } catch (error) {
    console.error("Error fetching loading plans:", error);
  }
