          ...
          <Menu as="div" class="flex-shrink-0 relative">
            <div>
              <MenuButton
                ...
                <span class="lowercase m-2 text-white"> {{ user?.firstname?.replace(/\./g, ' ') }} {{ user?.lastname?.replace(/\./g, ' ') }} </span>
                <span style="background-color:gray"
                  class="inline-flex items-center px-3 rounded-full text-sm font-medium text-white uppercase">
                  {{ user?.firstname?.match(/\b(\w)/g).join("") }}  {{ user?.lastname?.match(/\b(\w)/g).join("") }} 
                </span>
              </MenuButton>
            </div>
