      ...
      >
        ...
        <div class="flex items-center justify-between w-full lg:w-auto">
          ...
          <span class="font-bold text-white mx-4 hidden lg:block"
            >WFP CASE TRACKER | Co-operating Partner
  ...
  navList.forEach((navItem) => {
    const isMatched =
      ...
      (navItem.name === "Resources" &&
        (currentRouteBase.startsWith("/field/faqs") ||
          currentRouteBase.startsWith("/field/sops") ||
...
const onAbout = async () => {
  $router.push({ path: "/cp/about-system" });
};
...
const getCases = async () => {
  try {
    ...
    cases.push(...result.filter((item) => item.isRejected && item.district == user.value.district));
