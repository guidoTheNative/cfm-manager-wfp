      ...
      <div class="md:flex md:items-center md:justify-between">
        ...
        <div class="mt-4 flex-shrink-0 flex md:mt-0 md:ml-4">
          <button
            ...
            @click="sendCaseReminders"
          >
            Send Case Assignment Reminders
          </button>
        </div>
      </div>
        ...
        >
          <template #table-row="props">
            <span v-if="props.column.label === 'Options'">
              <router-link
                :to="{ path: '/activitymanager/cases/manage/' + props.row.id }"
                v-if="
...
const sendCaseReminders = async (model) => {
  /* isLoading.value = true;
  try {
    await caseStore.remindersSend(model);
    Swal.fire({
      title: "Success",
    ...
    Swal.fire({
      title: "Failed",
      text: "Failed to send case reminders ",
      icon: "error",
      confirmButtonText: "Ok",
    });
  } finally {
