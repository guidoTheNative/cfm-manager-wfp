        ...
        >
          <template #table-row="props">
            ...
            >
              <!-- Manage Button -->
              <router-link
                :to="`/callcenter/cases/manage/${props.row.id}`"
...
const getCases = async () => {
  ...
  try {
    ...
    // Replace "." with " " in username for comparison
    const formattedUsername = user.value.username.replace(/\./g, " ");

    cases.length = 0;
    cases.push(
      ...result
        .filter((caseItem) => caseItem.submittedBy == formattedUsername) // Filter cases
        .map((caseItem) => ({
          ...caseItem,
          created: moment(caseItem.created).format("DD-MM-YYYY"), // Format the date
          rawCreatedDate: moment(caseItem.created), // Store raw date for sorting
        }))
    );
