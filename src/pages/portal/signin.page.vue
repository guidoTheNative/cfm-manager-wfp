<template>
  <div class="min-h-full flex lg:flex-row" style="height: 100vh">
    <spinner-widget :open="isLoading" />

    <!-- Left Panel -->
    <div class="lg:w-1/3 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-12 xl:px-16 bg-white">
      <div class="mx-auto w-full max-w-sm">
        <div>
          <div class="flex justify-center items-center mb-4 lg:hidden rounded-md p-4" style="background-color: #096eb4;">
          ...
          <h2 class="mt-6 text-md font-extrabold text-gray-700 text-center lg:hidden">
            WFP CFM TRACKER <span class="text-sm font-normal">(v1.0)</span>
          </h2>
        ...
        <div class="mt-8">
          <form @submit="onSubmit" :validation-schema="schema" class="space-y-6">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                ...
                />
                <label for="show-password" class="ml-2 text-sm text-gray-900">Show password</label>
              </div>
              <router-link :to="{ name: 'portal-forgot-password' }"  style="color: #096eb4" class="text-sm">
          ...
          <div class="mt-10 text-center">
            <p class="text-sm text-gray-600">&copy; {{ moment().format('YYYY') }} {{ system.name }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Right Panel -->
    <div style="background-color: #096eb4" class="lg:w-2/3 flex flex-col justify-center items-center text-white px-8">
      <div class="text-center">
        <img class="h-24 mx-auto" src="../../assets/images/wfp-logo-emblem-white.png" alt="WFP" />
        <h2 class="mt-6 text-3xl font-extrabold">
          WFP CFM TRACKER <span class="text-sm font-normal">(v1.0)</span>
        </h2>
        <p v-if="system.mode === 'TRAINING'" class="mt-4 text-sm">({{ system.mode }})</p>
      </div>
    </div>
  </div>
</template>
...
const onSubmit = useSubmitForm((values, actions) => {
  ...
  sessionStore
    ...
    .then(async (result) => {
      ...
      console.log(result.role, "TEHHEHEH")
      
      // Check the role and redirect accordingly
      if (result.role === "Admin") {
        $router.push({ path: "/Admin" });
      } else if (result.role === "CFM manager") {
        $router.push({ path: "/cfmmanager" });
